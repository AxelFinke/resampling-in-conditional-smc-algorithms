...
)
{
 ...
 // std::cout << "Total number of loop iterations: " << M << std::endl;
 ...
 for (const auto& T: n_observations_values) {
   ...
   for (const auto& epsilon: epsilon_values) {
     for (const auto& delta: delta_values) {
       ...
       p = arma::exp(log_p);
       // std::cout << "Target density: " << p.t() << std::endl;
      //  std::cout << "log-normalising constant difference: " << evaluate_log_normalising_constant(path_space, epsilon, delta)
      //  - std::log(
      //    std::pow(1 - epsilon, 2) * std::pow(1 - delta, 2) +
      //    epsilon * (1 - epsilon) * delta * delta +
      //    (1 - epsilon) * epsilon * (1 - delta) * delta +
      //    epsilon * epsilon * delta * (1 - delta)
      // ) << std::endl; // NOTE: this only makes sense if T = 2
      ...
      for (const auto& N: n_particles_values) {
         for (const auto& resample_type: resample_type_values) {
           for (const auto& backward_sampling_type: backward_sampling_type_values) {
             ...
             // std::cout << "START: computing forward KLD!" << std::endl;
             forward_kld_full(m) = compute_kl_divergence(p, q);
             // std::cout << "START: computing reverse KLD!" << std::endl;
             reverse_kld_full(m) = compute_kl_divergence(q, p);
             // std::cout << "START: computing TVD!" << std::endl;
             tvd_full(m) = compute_tv_distance(p, q);
             // std::cout << "START: computing autocorrelation!" << std::endl;
             autocorrelation_full(m) = compute_autocorrelation(q, P, path_space, 0, 1);
             // std::cout << "END: computing autocorrelation!" << std::endl;
